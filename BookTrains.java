...
public class BookTrains extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			if (train != null) {
				int avail = train.getTotalseats();
				if (seat > avail) {
					pw.println("<div class='tab'><p1 class='menu red'>Only " + avail
							+ " Seats are Available</p1></div>");

				} else if (seat <= avail) {
					avail = avail - seat;
					train.setTotalseats(avail);
					...
					if (ResponseCode.SUCCESS.toString().equalsIgnoreCase(responseCode)) {
						...
						Double totalAmount = train.getTotalfare() * seat;
						bookingDetails.setAmount(totalAmount);
						bookingDetails.setDeparture(train.getDeparture());
						bookingDetails.setDestination(train.getDestination());
						bookingDetails.setTr_no(trainNo);
						bookingDetails.setTotalseats(seat);
						...
						pw.println("<div class='tab'>" + "<p class='menu'>" + "<table>"
								...
								+ "</td><td>Train Number: </td><td>" + transaction.getTr_no()
								+ "</td></tr><tr><td>Departure: </td><td>" + transaction.getDeparture()
								+ "</td><td>Destination:</td><td>" + transaction.getDestination() + "</td></tr>"
								+ "<tr><td>Date Of Journey:</td><td>" + transaction.getDate()
								+ "</td><td>Time(HH:MM):</td><td>11:23</td></tr><tr><td>Passangers: </td><td>"
								+ transaction.getTotalseats() + "</td><td>Class: </td><td>" + seatClass + "</td></tr>"
