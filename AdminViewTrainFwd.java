	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			if (trains != null && !trains.isEmpty()) {
				...
				rd.include(req, res);
				pw.println("<div class='main'><p1 class='menu'>Ongoing Trains</p1></div>");
				pw.println("<div class='tab'><table><tr><th>Train Name</th><th>Train Number</th>"
						+ "<th>From Station</th><th>To Station</th><th>Total Seats Available</th><th>Total Fare(INR)</th><th>Action</th></tr>");

				for (TrainBean train : trains) {

					pw.println("" + "<tr> " + "" + "<td><a href='viewadmin?trainNo=" + train.getTr_no() + "&fromStn="
							+ train.getDeparture() + "&toStn=" + train.getDestination() + "'>" + train.getTr_name()
							+ "</a></td>" + "<td>" + train.getTr_no() + "</td>" + "<td>" + train.getDeparture() + "</td>"
							+ "<td>" + train.getDestination() + "</td>" + "<td>" + train.getTotalseats() + "</td>" + "<td>"
							+ train.getTotalfare() + " RS</td>" + "<td><a href='adminupdatetrain?trainnumber="
