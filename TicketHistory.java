...
public class TicketBookingHistory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			List<HistoryBean> details = bookingService.getAllBookingsByClientId(clientId);
			if (details != null && !details.isEmpty()) {
				RequestDispatcher rd = req.getRequestDispatcher("UserViewTrains.html");
				rd.include(req, res);
				pw.println("<div class='main'><p1 class='menu'>Ticket History</p1></div>");
				...
				for (HistoryBean trans : details) {
					...
					pw.println("" + "<tr> " + "" + "<td>" + trans.getTransId() + "</td>" + "<td>" + trans.getTr_no()
							+ "</td>" + "<td>" + trans.getDeparture() + "</td>" + "<td>" + trans.getDestination() + "</td>"
							+ "<td>" + trans.getDate() + "</td>" + "<td>" + trans.getTotalseats() + "</td><td>"
